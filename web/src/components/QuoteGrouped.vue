...
q-card(class="list-group-item" :bordered="!simple" flat).text-left.q-pa-none
  ...
  q-card-section(horizontal)
    q-card-section(side v-if="highlight")
      q-checkbox(:model-value="element.quotes[0].highlighted || false" @update:model-value="updateHigh(element,$event)")  
...
export default defineComponent({
  ...
  computed: {
    text() {
      ...
      return map(this.element.quotes, "alternatives[0].transcript").join(" ");
    },
  },
  methods: {
